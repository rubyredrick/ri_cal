...
module RiCal
  class PropertyValue
    ...
    class RecurrenceRule < PropertyValue
      ...
      def advance_days(time, enumerator, debug = false) # :nodoc:
        if freq == 'DAILY'
          ...
          if by_day_rules && by_day_rules.first.scope == :daily
            ...
            if_more_in_by_rule_list(:byday, time, enumerator) {|t| return t}
            if_more_in_by_rule_list(:bymonthday, time, enumerator) {|t| return t}
            if_more_in_by_rule_list(:byyearday, time, enumerator) {|t| return t}
          end
      ...
      def if_more_in_by_rule_list(rule_type, time, enumerator)
      ...
      def by_day_scope(by_lists_hash)
         case freq
        when "YEARLY"
          scope = :yearly
      ...
      def set_by_lists(value_hash)
        [:bysecond,
          ...
          if val = value_hash[:byday]
            ...
            by_list[:byday] = [val].flatten.map {|day| RecurringDay.new(day, self, by_day_scope(value_hash))}
          end
