    ...
    class RecurrenceRule < PropertyValue
      ...
      def advance_days(time, enumerator, debug = false) # :nodoc:
        if freq == 'DAILY'
          ...
          next_by_daily_scoped_byday_rule = next_for_daily_scoped_byday_rule(time)
          if next_by_daily_scoped_byday_rule
            return next_by_daily_scoped_byday_rule
          elsif (next_bypart = next_for_enumerating_bypart(time, enumerator) || enumerator.start_of_next_scope)
            return next_bypart
          else
      ...
      def next_for_enumerating_bypart(time, enumerator)
        by_part_to_enumerate = enumerating_bypart
        if by_part_to_enumerate 
          next_bypart = next_in_by_rule_list(by_part_to_enumerate, time, enumerator)
        else
          nil
        end
      end
      
      def enumerating_bypart
        (@by_day_scope == :yearly ? 
        [:byyearday, :byday, :bymonthday] : 
        [:byyearday, :bymonthday, :byday]).find {|rule| by_rule_list(rule)}
      end
      
      def next_for_daily_scoped_byday_rule(time)
        by_day_rules = by_rule_list(:byday)
        if by_day_rules && by_day_rules.first.scope == :daily
          new_time = time.advance(:days => 1)
          if freq == "WEEKLY" && interval > 1 && new_time.wday == wkst_day
      ...
      # Look for a time generated by a particular byxxx rule
      # yield it if one is found, the caller should pass a block which returns
      def next_in_by_rule_list(rule_type, time, enumerator)
