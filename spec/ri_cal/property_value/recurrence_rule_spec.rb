    ...
    def self.enumeration_spec(description, dtstart_string, tzid, rrule_string, expectation, debug=false)
      ...
      describe description do
        before(:each) do
          ...
          default_start_time = RiCal::PropertyValue::DateTime.new(:value => dtstart_string, :tzid => tzid)
          @enum = rrule.enumerator(mock("EventValue", :default_start_time => default_start_time, :default_duration => nil))
          @expectations = (expectation.map {|str| RiCal::PropertyValue::DateTime.new(:value => str.gsub(/E.T$/,''), :tzid => tzid)})
        end
